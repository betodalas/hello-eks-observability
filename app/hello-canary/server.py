...
class Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/healthz":
            ...
            lines = [
                "# HELP hello_canary_requests_total Total HTTP requests by status code",
                "# TYPE hello_canary_requests_total counter",
            ]
            for code, count in sorted(REQUESTS_BY_CODE.items()):
                lines.append(f'hello_canary_requests_total{{code="{code}"}} {count}')
            body = ("\n".join(lines) + "\n").encode()
        else:
            REQUESTS_BY_CODE[200] += 1
            body = b"Hello from hello-canary, novo app para testar o rollout!\n"
